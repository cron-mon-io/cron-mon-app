...
export class AlertConfigRepository extends ApiRepository implements AlertConfigRepoInterface {
    ...
    async getAlertConfig(alertConfigId: string): Promise<AlertConfig> {
        const resp = await this.sendRequest(`/api/v1/alert-config/${alertConfigId}`, 'GET')
        return (resp as AlertConfigResp).data
    }

    async addAlertConfig(alertConfig: AlertConfig): Promise<AlertConfig> {
        return await this.postAlertConfigInfo(`/api/v1/alert-config`, 'POST', alertConfig)
    }

    async updateAlertConfig(alertConfig: AlertConfig): Promise<AlertConfig> {
        return await this.postAlertConfigInfo(
            `/api/v1/alert-config/${alertConfig.alert_config_id}`, 'PATCH', alertConfig
        )
    }

    async deleteAlertConfig(alertConfig: AlertConfig): Promise<void> {
        await this.sendRequest(`/api/v1/alert-config/${alertConfig.alert_config_id}`, 'DELETE')
    }
