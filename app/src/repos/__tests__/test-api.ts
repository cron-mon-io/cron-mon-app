...
export function setupTestAPI(): SetupServer {
  const apiHandlers = [
    ...
    http.post('http://127.0.0.1:8000/api/v1/monitors', async ({ request }) => {
      const body = (await request.json()) as BasicMonitorInformation
    ...
    http.patch('http://127.0.0.1:8000/api/v1/monitors/:monitorId', async ({ params, request }) => {
      const { monitorId } = params
      const monitor = MONITORS.find((m) => m.monitor_id === monitorId)
  ...
  return setupServer(...apiHandlers)
}
